...
{	var filas = obtener_valor('filMT_REEMP');
	...
	if (! MatrizVacia)
	{
		...
		{	
			...
			if (obtener_valorM('MT_REEMP',  i, 3) == '') 
			...
			if (obtener_valorM('MT_REEMP',  i, 1) == obtener_valorM('MT_REEMP',  i, 3)) 
			{	alertmb("Los puestos no pueden ser iguales, línea " + i, 2, 1, "Aceptar");
				return false;				
			}	
			for (j = i + 1; j <= filas; j++)
			{	if (obtener_valorM('MT_REEMP', i, 2) == obtener_valorM('MT_REEMP', j, 2))

...
try{
	...
	for(i=0;i<longitudPila;i++){
		if ((PilaEvents[i].tipoE==tipoE)&&(PilaEvents[i].campoP==campoP)&&(PilaEvents[i].orden==orden)){
...
////				if (IE){
					...
					mat["mensaje"] ="Manejo Cliente: La rutina: ["+PilaEvents[i].nbfuncion+"] retornó un valor no válido. Los valores posibles son true o false.";
					mat["icono"] = "images/Alerta.gif";
					doDialog(mat);
////				}else{
////					alert("Manejo Cliente: La rutina: ["+PilaEvents[i].nbfuncion+"] retornó un valor no válido. Los valores posibles son true o false.");
////				}	
				retorno = false;
				break;
			}else if (!ret){			
	...
	if (document.getElementById("M"+fila+campoP+columna).tagName=="SELECT"){
		...
		for(i=0;i<num_list;i++){
			if (document.getElementById("M"+fila+campoP+columna).item(i).getAttribute("value")){
				if (document.getElementById("M"+fila+campoP+columna).item(i).getAttribute("value")==valor){
					...
					document.getElementById("M"+fila+campoP+columna).onchange();
					break;	
				}
			}
		...
		try{
			if (document.getElementById("M"+fila+campoP+columna).type=="checkbox"){
				...
				document.getElementById("M"+fila+campoP+columna).value = valor;
				document.getElementById("M"+fila+campoP+columna).onchange();
			}
		}catch(e){}
	}	
	    ...
	    if (document.getElementById(campoP).tagName=="SELECT"){
			...
			if (index==0){		
				if (document.getElementById(campoP).item(document.getElementById(campoP).selectedIndex).getAttribute("value")){
					return(document.getElementById(campoP).item(document.getElementById(campoP).selectedIndex).getAttribute("value"));
				}else{
...
function obtener_valorM(campoP,fila,columna,tipo){
	try{
	    ...
	    if (document.getElementById("M"+fila+campoP+columna).tagName=="SELECT"){
			if (tipo==0){
				//if (document.getElementById("M"+fila+campoP+columna).item(document.getElementById("M"+fila+campoP+columna).selectedIndex).getAttribute("value")){

...
//FUNCION GENERICA QUE ASIGNA NUMERO DE FILA UNA POR UNA
...
//FUNCION GENERICA QUE ASIGNA NUMERO DE FILA DE LA MATRIZ COMPLETA
function AsignarNroFila(mat){
  var fil = obtener_valor( 'fil'+mat );
  for(e=1;e<=fil;e++){
	asignar_valorM( mat , e ,  e, 1);
  }
    return true;
}
...
function validarResponsableIMSS(columna){
	var matriz="MTZDET";
	var url = obtener_valor('URL');
	var fila = get_fila(matriz);
	if (obtener_valorM( "MTZDET", fila, columna)!="" ){
	var idEmpresa = obtener_valorM(matriz,fila,columna);
	var inValida=false;
	if($("#M"+fila+matriz+"12").attr('checked')== true){
		$.ajax({
			type: "GET",
		...
		if (!inValida){
			$("#M"+fila+matriz+"12").attr("checked", false);
...
function validarResponsableIMSS2(columna){
	var matriz="MTZDET";
	var url = obtener_valor('URL');
	var fila = get_fila(matriz);
	if (obtener_valorM( "MTZDET", fila, columna)!="" ){
	...
	if($("#M"+fila+matriz+"12").attr('checked')== true){
		...
		if (!inValida){
			alertmb("La empresa pagadora seleccionada no maneja IMSS",0,1,"Aceptar");
			asignar_valorM( "MTZDET" ,"Seleccione" , fila, columna); 
		}
	}
	}
	return true;

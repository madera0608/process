﻿(function(){
    ...
    function ticketService($window) {
        ...
        function numReq(numReq) {
            $window.localStorage['numReq'] = numReq;
        }
        
        function getNumReq() {
        	var num = $window.localStorage['numReq']; 
        	if (num){
        		return num;	
        	}else{        		
        		return 0;
        ...
        function resetListError() {
        	$window.localStorage.removeItem('listMessageError');
        	$window.localStorage.removeItem('listStatusCode');//forzar el reseteo de los mensajes
        	numReq(0);
        }        
        
        function listStatusCode(code) {
        	if ($window.localStorage['listStatusCode']){
        		var listStatus = JSON.parse($window.localStorage['listStatusCode']);        		
        		var listMessage = JSON.parse($window.localStorage['listMessageError']);
        		var inList = false;
        		 ...
        		 if (!inList){        			 
                    ...
                    $window.localStorage['listStatusCode'] = JSON.stringify(listStatus);
                    $window.localStorage['listMessageError'] = JSON.stringify(listMessage);               		
        		 }        		
        	}else{
        		var listStatus = [];
        		var listMessage = [];
        ...
        function isShowError() {
            var error = $window.localStorage['listStatusCode'];
            if (error) {
                return true;
            } else {
                return false;
    ...
    function processEngine($http, API, $rootScope){
        ...
        function changePassApprovalProfile(vm) {
            return $http.put(API + '/user/updatesecurity?ctx=1&clave1=' + vm.user.approval + '&clave2=' + vm.profile.pregunta + '&clave3=' + vm.user.answer)
        ...
        function changeSecurityQuestionProfile(vm) {
            return $http.put(API + '/user/updatesecurity?ctx=0&clave1=' + vm.user.question + '&clave2=' + vm.user.answer + '&clave3=' + vm.user.approval)
        ...
        function createDocument(vm){
            return $http.post(API + '/document?wfp=' + vm.wfp + '&frmn=' + vm.frmn + '&env=' + $rootScope.nbAmbiente)
        ...
        function openDocument(vm){
            return $http.get(API + '/document/abrir?nudoc=' + vm.nuDoc + '&nuinst=' + vm.nuInst+ '&wfa=' + vm.wfa + '&env=' + $rootScope.nbAmbiente)
        ...
        function getDocument(vm){
            return $http.get(API + '/document?frmn=' + vm.frmn)
        ...
        function saveFormDocument(vm) {
            return $http.put(API + '/document/saveform?frmn='+vm.docData.forms[0].frmn,angular.toJson(vm.docData.forms[0], false))
...
//            	} 
        ...
        function resolveListEventAgent(vm) {
            return $http.post(API + '/document/resolverlista?campo=' + vm.eventCampo + '&fila=' + vm.eventFila + '&sel=' + vm.eventSelList + '&ctx=' + vm.eventCtx, vm.eventXml )
        ...
        function openDocumentRead(vm){
            return $http.get(API + '/document/abrirlectura?nudoc=' + vm.nuDoc + '&nuinst=' + vm.nuInst + '&env=' + $rootScope.nbAmbiente)

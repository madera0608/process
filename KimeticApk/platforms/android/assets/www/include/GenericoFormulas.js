...
{
//Esta función busca en el arreglo de campos cuales son los que
...
//Función que elimina las comas de un float
//Parámetros:
//	pe_valor: valor de entrada
//      pe_sepdec: separador de decimal especificado en el servidor 
//Retorna: el valor del float si el número es válido
//         cero si el número es inválido
//Desarrollado por: Johann Briceño.  Fecha: 14/07/1999
...
//         3 = El valor minimo es mayor que el valor maximo, error en el diseño     
...
var FormulasGenerales=new Array(Array(),Array());  //Contiene las formulas genéricas para reconstruir matrices cuando se eliminen filas
var ind=0;

function RegistrarFormula(Formula,CampoActualizar,FilaInicial)
//Esta función desglosa la formula ingresada para registrar
...
//se deben actualizar
{
	...
	if(CampoActualizar.indexOf("[")>0)//Aqui entra cuando hay que actualizar una columna de una matriz
	{
		if(Formula.indexOf("[")>0)
		{
			...
			var Columna = new Number();//Contiene la columna de la matriz que será actualizada
	...
	while(punto1<Formula.length)
	{
		...
		CamposForm[i]=Formula.substring(punto1+1,punto2)+"";

		CamposForm[i]=CamposForm[i].toString().replace("(","");
		CamposForm[i]=CamposForm[i].toString().replace(")","");
		if(CamposForm[i].length>0&&punto1>=0)
		{
			punto1=punto2+1;
			
			if(CamposForm[i].toString().indexOf("[")<0)//Para cuando se trata de un campo externo a una matriz
			{
				CrearFuncionCampoCambio(CamposForm[i]+"");
				if(CampoActualizar.indexOf("[")>0)//Para cuando se debe actualizar una Matriz
				{
					var campo=new String();
					campo=CampoActualizar.substring(0,CampoActualizar.indexOf("["));
					var filas=document.getElementById("fil"+campo).value;
					var columna=CampoActualizar.substring(CampoActualizar.indexOf(",")+1 , CampoActualizar.indexOf("]"));					
					
					for(var y=1;y<=filas;y++)//Guarda la relación por cada fila de la matriz a actualizar
						GuardarRelacionCampos(CamposForm[i],"M"+y+campo+columna);
				}
				else //Cuando se actualiza un campo externo a una matriz
					GuardarRelacionCampos(CamposForm[i],CampoActualizar);
			}
			else//Para cuando es una matriz
			{
				var campo=new String();
				campo=CamposForm[i].toString().substring(0,CamposForm[i].toString().indexOf("["));
				var filas=document.getElementById("fil"+campo).value;
				var columna=CamposForm[i].toString().substring(CamposForm[i].toString().indexOf(",")+1 , CamposForm[i].toString().indexOf("]"));
				
				for(var j=1;j<=filas;j++)
				{
					if(CampoActualizar.indexOf("*")>0)//Cuando se debe actualizar la columna de una matriz
					{
						var MatCol=new String();
						var SecCol=new String();
						var frm=new String();	//Contendrá la formula para hacer los reemplazos necesarios
						CrearFuncionCampoCambio("M"+j+campo+columna);

						var pcampo=new String();
						frm=Formula.replace(":"+campo+"[*,"+columna+"]:",":M"+j+campo+columna+":");

						var oc=0;
						var z=0;
						
						//Modifica la formula original para adaptarla a cada fila y columna
						while(frm.indexOf("[")>0&&z<50)
						{
							oc=frm.substring(frm.indexOf("*,")+2,frm.indexOf("]"));
							frm=frm.replace(":"+campo+"[*,"+oc+"]:",":M"+j+campo+oc+":");
							z++;
						}	
						MatCol=CampoActualizar.substring(0,CampoActualizar.indexOf("["))+"";
						SecCol=CampoActualizar.substring(CampoActualizar.indexOf(",")+1,CampoActualizar.indexOf("]"));
						if(MatCol==campo)
						{
							GuardarRelacionCampos("M"+j+campo+columna,"M"+j+MatCol+SecCol);
							GuardarFormula("M"+j+MatCol+SecCol,frm);
							RegFor=false;
						}
						else
							alert("No se permiten referencias de matrices cruzadas");
						
					}
					else//Para actualizar un campo
					{
						CrearFuncionCampoCambio("M"+j+campo+columna);
						GuardarRelacionCampos("M"+j+campo+columna,CampoActualizar);
						alert(Formula);
					}
				}
	...
	if(RegFor)
		GuardarFormula(CampoActualizar,Formula);
	*/
	//alert(ArregloCampos[0].join("\n")+"\n\n"+ArregloCampos[1].join("\n"));
	return(true);
}
...
if(!Enc)//Si no se encontró la relación se agrega
{
			...
			while(Formula.indexOf(":")>=0)
			{
				...
				if(campo.indexOf("[")>0)//Para una matriz
				{
					if(campo.indexOf("{+}")>0)
					{
						...
						for(x=1;x<=filas;x++)
						{							
							valmat=document.getElementById("M"+x+campo.substring(0,campo.indexOf("["))+columna).value;
...
{
//Devuelve una cadena con los campos que dependen del indicado como parámetro
var cadena = new String();
var CadRet=new String();
cadena=Campos+"";
var Camps=new Array();
...
{
//Busca los campos que dependen de NombreCampo y ejecuta la función de Cambio para esos campos
	var i=0;
	var j=0;
	var punto1=0;
	var punto2=0;

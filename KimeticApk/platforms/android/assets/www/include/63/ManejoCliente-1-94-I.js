$("#M1RIF1").change(
function() { 
	if (obtener_valor("M1RIF1") == "J" || obtener_valor("M1RIF1") == "G"){
		asignar_valor("TPOPERS", "T",1);}
	else{
		asignar_valor("TPOPERS", "T",0);}
	}
);

HabilitaCampos("FEMAXINI","",[[2,"wl_bsolicitar"], [2,"wl_bsolicitar1"], [4,"DATOS DEL SOLICITANTE"], [4,"IDENTIFICACIÓN DEL RECLAMO"], [4,"INFORMACIÓN DE LOS RECAUDOS"], [4,"DATOS DE LA RECEPCIÓN DEL ANALISIS"]]);

$("#FEMAXINI").change(function() { HabilitaCampos("FEMAXINI","",[[4,"DATOS DEL SOLICITANTE"], [4,"IDENTIFICACIÓN DEL RECLAMO"], [4,"INFORMACIÓN DE LOS RECAUDOS"], [4,"DATOS DE LA RECEPCIÓN DEL ANALISIS"]]); } );

$("#TPOSOLCITU").change(function() { Conectarlink1('Agente.asp?wl_eventoagente=true&wl_campof=TPOSOLCITU&wl_filaf=0'); } );
$("#evento_TPOSOLCITU").hide();

//EVENTO QUE CARGA ESTADOS Y CIUDADES

function act_municipios() {
	 var cod_estado = obtener_valor( "DIR_EDO" );

	 $.ajax({ type:"GET",
	 url:"",
	 data:"serv=1&cod="+cod_estado,
...
asignar_event( "onchange" , "act_municipios()" , "DIR_EDO" );

function act_ciudades() {

	 var cod_estado = obtener_valor( "DIR_EDO" );

	 $.ajax({ type:"GET",
	 url:"AJAX/AjaxService.asp",
	 data:"serv=2&cod="+cod_estado,
	 success: function(data) {
...
asignar_event( "onchange" , "act_ciudades()" , "DIR_EDO" );

//

function ValidaTRAN()
{
  if (obtener_valorM('TRANSHIS',1,1)=='' && obtener_valor('ES_TRANSAC',0)=='T' )
  {
    alertmb( 'La matriz de Transacción a reclamar no debe estar vacía', 2 , 1 ,  "Aceptar");   
    return false;
   }  
 return true;
}

asignar_event("onclick" , "ValidaTRAN()", "wl_bsolicitar");
asignar_event("onclick" , "ValidaTRAN()", "wl_bsolicitar1");
...
function mostrar_fila(){ 
    ...
    asignar_valorM( "TRANSHIS" , NUFILA , 1 , 8 );
     ...
     var abono = obtener_valorM( "TRANSHIS" , 1 , 3);
     asignar_valor("MTO_ABONO" , abono);
...
asignar_valorM("TRANSAC","1",1,1);
asignar_valorM("TRANSAC","08/11/2011",1,2);
asignar_valorM("TRANSAC","54,00",1,3);
asignar_valorM("TRANSAC","1",1,4);
asignar_valorM("TRANSAC","1",1,5);
asignar_valorM("TRANSAC","TDC",1,6);
asignar_valorM("TRANSAC","Caracas",1,7);
asignar_valorM("TRANSAC","CARONI",1,8);

asignar_valor("MENSAJE", "El Mensaje");
/******************************************************/
